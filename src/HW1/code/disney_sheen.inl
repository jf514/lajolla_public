...
Vector3 F_sheen(const Vector3& baseColor, Real sheenTint, const Vector3& norm,
    ...
    Real fac = pow(1.0f - std::abs(dot(h, dir_out)), 5.0f)*fabs(dot(norm, dir_out));
...
Spectrum eval_op::operator()(const DisneySheen &bsdf) const {
    ...
    // Homework 1: implement this!
    Vector3 res = F_sheen(baseColor, sheenTint, frame.n, dir_in, dir_out);
