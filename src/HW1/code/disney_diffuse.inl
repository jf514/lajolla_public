...
Vector3 F_diffuse(const Vector3& baseColor, 
    const Vector3& norm, const Vector3& dir_in, const Vector3& dir_out,
    Real roughness, Real subsurface){

    Vector3 h = normalize(dir_in + dir_out);
    Real FD_in = FD(norm, dir_in, dir_out, h, roughness);
    Real FD_out = FD(norm, dir_out, dir_out, h, roughness);
    
    Real f_base_diffuse = (FD_in * FD_out/c_PI) * 
        fabs(dot(norm, dir_out));

    // Subsurface
    Real FSS_comb = FSS_Combined(norm, dir_in, dir_out, h, roughness);
...
Spectrum eval_op::operator()(const DisneyDiffuse &bsdf) const {
    ...
    Vector3 res = F_diffuse(baseColor, frame.n, dir_in, dir_out, roughness, subsurface);
