...
Vector3 F_glass(Real anisotropic, Real roughness, Real eta, const Vector3& vtx_normal,
    const TransportDirection& dir,
    const Frame& frame, const Vector3& baseColor, const Vector3& dir_in, const Vector3& dir_out){

    bool reflect = dot(vtx_normal, dir_in) *
                   dot(vtx_normal, dir_out) > 0;

    eta = dot(vtx_normal, dir_in) > 0 ? eta : 1 / eta;
...
Spectrum eval_op::operator()(const DisneyGlass &bsdf) const {
    ...
    Vector3 res2 = F_glass(anisotropic, roughness, bsdf.eta, vertex.geometric_normal, dir, frame,
